...
|--------------------------------------------------------------------------
|
| Here are all the admin-specific routes. These routes are loaded by the
| RouteServiceProvider and all of them will be assigned to the "admin" middleware group.
| All routes here will have /admin prefix and admin. name prefix.
|
*/

// User Management Routes
Route::group(['prefix' => 'users', 'as' => 'users.'], function () {
    Route::get('/', [RolePermissionController::class, 'users'])
        ->middleware('permission:view users')
        ->name('index');
    
    Route::patch('/{user}/toggle-status', [RolePermissionController::class, 'toggleUserStatus'])
        ->middleware('permission:edit users')
        ->name('toggle-status');
    
    Route::patch('/{user}', [RolePermissionController::class, 'updateUser'])
        ->middleware('permission:edit users')
        ->name('update');
    
    Route::delete('/{user}', [RolePermissionController::class, 'deleteUser'])
    ...
    Route::delete('/{user}/remove-role', [RolePermissionController::class, 'removeRole'])
        ->middleware('permission:assign roles')
        ->name('remove-role');
    ...
    Route::delete('/{user}/remove-permission', [RolePermissionController::class, 'removePermission'])
        ->middleware('permission:assign permissions')
...
Route::group(['prefix' => 'teams', 'as' => 'teams.'], function () {
    ...
    Route::post('/', [\App\Http\Controllers\TeamController::class, 'store'])
        ->middleware('permission:create teams')
        ->name('store');
    
    Route::patch('/{team}/toggle-active', [\App\Http\Controllers\TeamController::class, 'toggleActive'])
        ->middleware('permission:edit teams')
        ->name('toggleActive');
    
    Route::delete('/{team}', [\App\Http\Controllers\TeamController::class, 'destroy'])
        ->middleware('permission:delete teams')
        ->name('destroy');
});
